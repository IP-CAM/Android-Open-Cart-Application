...
import com.oshop.hm.adapters.ProductAdapter;
import com.oshop.hm.customs.EndlessRecyclerViewScroll;
...
public class All_Products extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable final ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // if saved from another Fragments
            ...
            public void onResponse(Call<ProductData> call, retrofit2.Response<ProductData> response) {
                ...
                if (response.isSuccessful()) {
                    if (response.body().getSuccess().equalsIgnoreCase("1")) {
                        /*Toast.makeText(getActivity(), "Success " + response.body().getProductData().size()  , Toast.LENGTH_LONG).show();
                        for ( int i = 0 ;i <  response.body().getProductData().size() ; i++ ) {
                            Log.w("foreach==" , response.body().getProductData().get(i).getProductsName());
                            Log.w("foreach==" , response.body().getProductData().get(i).getProductsImage());
                            Log.w("foreach==" ,"----------");
                        }*/
