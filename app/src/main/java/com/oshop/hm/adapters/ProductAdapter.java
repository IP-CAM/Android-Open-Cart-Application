...
//                holder.product_price_new.setText(ConstantValues.CURRENCY_SYMBOL + product.getDiscountPrice());
                ...
                //old one
                float aed_price_old = myAppPrefsManager.getAedCurrencyRate() * Utilities.parseArabicFloat(product.getProductsPrice());
                aed_price_old = Utilities.round(aed_price_old,2);
                // new_one
                float aed_price = myAppPrefsManager.getAedCurrencyRate() * Utilities.parseArabicFloat(product.getDiscountPrice());
                aed_price = Utilities.round(aed_price,2);
//                holder.product_price_aed.setText(aed_symbol + aed_price);
                if(ConstantValues.CURRENCY_TYPE == "AED") {
                    // aed currency
                    holder.product_price_old.setText(aed_symbol + aed_price);
                    holder.product_price_new.setText(aed_symbol + aed_price);
                }else{
                    // dollar currency
                    holder.product_price_old.setText(ConstantValues.CURRENCY_SYMBOL + product.getProductsPrice());
                    holder.product_price_new.setText(ConstantValues.CURRENCY_SYMBOL + product.getDiscountPrice());
...
//                holder.product_price_new.setText(ConstantValues.CURRENCY_SYMBOL + product.getProductsPrice());
                ...
                //New
                String aed_symbol = myAppPrefsManager.getAedCurrencySymbol();
                float aed_price = myAppPrefsManager.getAedCurrencyRate() * Utilities.parseArabicFloat(product.getProductsPrice());
                aed_price = Utilities.round(aed_price,2);
//                holder.product_price_aed.setText(aed_symbol + aed_price);
                if(ConstantValues.CURRENCY_TYPE == "AED") {
                    // aed currency
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(final View itemView) {
            ...
            // new
            product_price_aed = (TextView) itemView.findViewById(R.id.product_price_aed);
    ...
    private void addProductToCart(ProductDetails product) {
        ...
        if (discount != null) {
            ...
            productBasePrice = Utilities.parseArabicDouble(product.getDiscountPrice());
        } else {
            product.setIsSaleProduct("0");
            productBasePrice = Utilities.parseArabicDouble(product.getProductsPrice());
        }
