...
import com.oshop.hm.databases.User_Recents_DB;
import com.oshop.hm.fragments.My_Cart;
import com.oshop.hm.fragments.Product_Description;
...
public class CartItemsAdapter extends RecyclerView.Adapter<CartItemsAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        if (position == cartItemsList.size()) {
            ...
            //New
            String product_price_base_aed = "" , product_price_sub_aed = "" ;
            try {
                ...
                float base_price = Utilities.parseArabicFloat(cartProduct.getCustomersBasketProduct().getProductsPrice());
                float sub_price = Utilities.parseArabicFloat(cartProduct.getCustomersBasketProduct().getTotalPrice());

                float aed_base_price = myAppPrefsManager.getAedCurrencyRate() * base_price;
                float aed_sub_price = myAppPrefsManager.getAedCurrencyRate() * sub_price;

                aed_base_price = Utilities.round(aed_base_price, 2);
                aed_sub_price = Utilities.round(aed_sub_price, 2);

                product_price_base_aed =  aed_symbol + aed_base_price ;
                product_price_sub_aed =  aed_symbol + aed_sub_price ;
            }catch (Exception error){
                product_price_base_aed = "";
                product_price_sub_aed = "";

            }
            holder.cart_item_base_price_aed.setText(product_price_base_aed);
            holder.cart_item_sub_price_aed.setText(product_price_sub_aed);
            //New
            ...
            holder.cart_item_quantity_minusBtn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (number[0] > 1)
                    {
                        ...
                        double price = Utilities.parseArabicDouble(cartProduct.getCustomersBasketProduct().getProductsFinalPrice()) * number[0];
    ...
    public void setCartTotal() {
        ...
        for (int i=0;  i<cartItemsList.size();  i++) {
            ...
            finalPrice += Utilities.parseArabicDouble(cartItemsList.get(i).getCustomersBasketProduct().getTotalPrice());
        }

        //New
        String product_price_aed = "" ;
        try {
            MyAppPrefsManager myAppPrefsManager = new MyAppPrefsManager(context);
            String aed_symbol = myAppPrefsManager.getAedCurrencySymbol();
            float aed_price = myAppPrefsManager.getAedCurrencyRate() * (float) finalPrice;//Utilities.parseArabicFloat(product.getDiscountPrice());
            aed_price = Utilities.round(aed_price, 2);
            product_price_aed = "   " + aed_symbol + aed_price;
        }catch (Exception error){
            product_price_aed = "";
        }
        //New

        cartFragment.cart_total_price.setText(context.getString(R.string.total) +" : "+ ConstantValues.CURRENCY_SYMBOL + new DecimalFormat("#0.00").format(finalPrice) + product_price_aed);
    }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView cart_item_title, cart_item_category, cart_item_base_price,cart_item_base_price_aed
                , cart_item_sub_price , cart_item_sub_price_aed, cart_item_quantity;
        ...
        public MyViewHolder(final View itemView) {
            ...
            cart_item_base_price_aed = (TextView) itemView.findViewById(R.id.cart_item_base_price_aed);
            cart_item_sub_price = (TextView) itemView.findViewById(R.id.cart_item_sub_price);
            cart_item_sub_price_aed = (TextView) itemView.findViewById(R.id.cart_item_sub_price_aed);
